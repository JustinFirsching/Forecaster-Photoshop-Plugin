...
$._MYFUNCTIONS = {
  ...
  getFilesInfo: function(){
    var cb = JSON.stringify(getFilesInformation())
    return cb
  },
...
function getFilesInformation(){
  ...
  for (var i = 0; i < app.documents.length; i++) {
    if (app.documents[i].name.toLowerCase().indexOf('englewood') != -1){
      arr.push({documentNum: i, fileName: 'englewood'})
    }
    if (app.documents[i].name.toLowerCase().indexOf('venice') != -1){
      arr.push({documentNum: i, fileName: 'venice'})
    }
  }
...
try {
  params.forEach(function(data){
    ...
    var name = doc.name.split('_')[0] + data.date
